...
# 1. Load the dataset (adjust sep if needed)
df = pd.read_csv('Employee_data_v2.csv')

# 2. Peek at the first few rows to verify it loaded correctly
print(df.head())

# 3. Check for missing values to ensure our stats won't be skewed
print("\nMissing values per column:")
print(df.isnull().sum())

# 4. Print df.info() to see dtypes and non‐null counts
print("\nDataFrame info:")
print(df.info())

# 5. Specify which column is categorical and which are numeric
cat_col  = 'Education'
num_cols = ['JoiningYear', 'PaymentTier', 'Age', 'ExperienceInCurrentDomain']

# 6. Compute and round summary stats for each category
grouped_stats = (
    df
...
# 7. Convert the categorical column to pandas.Categorical, then get integer codes
